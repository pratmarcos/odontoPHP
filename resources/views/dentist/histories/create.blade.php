...
@section('title', 'Agregar al historico')
...
@endif
	...
	{!! Form::open(['route' => ['histories.store', $customer], 'method' => 'POST']) !!}
...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-5">
			...
			<div class="form-group">
				<select name="grupos" id="main" class="principal">
					<label class="hidden">{{$var = 0}}</label>
					@foreach ($groupstreatments as $grouptreatment)
						<label class="hidden">{{$var = $var+1}}</label>
						<option value="{{$var}}">{{$grouptreatment->name}}</option>
					@endforeach
				</select >
				...
				@foreach($groupstreatments as $grouptreatment)
					<label class="hidden">{{$var = $var+1}}</label>
					<select name="treatment_id" id="sub_{{$var}}" class="sub">
					@foreach($treatments as $treatment)
						@if($grouptreatment->id == $treatment->grouptreatment_id)
							<option>{{$treatment->process}}</option>
						@endif
					@endforeach
					</select>
				@endforeach
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td>
							<a class="btn btn-success" href="{{route('histories.edit',[$history->id, $history->tooth ])}}" title="Editar cliente" role="button"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
...
<script type="text/javascript">
	...
	function mostrarSelect() {
		...
		$("#sub_" + $(".principal").val()).addClass("on");
	}
